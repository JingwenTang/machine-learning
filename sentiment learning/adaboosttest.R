...
d3 = data.frame(cbind(id,prednewy))
write.table(d3,'E:/third-2nd/机器学习概论/exp2/prediction6.txt',sep = ',',row.names = F,col.names = c("id","label"))
...
DPrednewy = matrix(0,nrow = length(id),ncol = DK)
Der = rep(0,DK)
Dbeta = rep(0,DK)
...
for (t in 1: DK){
  ...
  DPrednewy[,t] = predict(DT,as.data.frame(Ttdm))
...
d4 = data.frame(cbind(id,Dprednewy))
write.table(d4,'E:/third-2nd/机器学习概论/exp2/prediction4.txt',sep = ',',row.names = F,col.names = c("id","label"))
